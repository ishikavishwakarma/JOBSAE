...
const CategorySection = ({ industries, onNavigate }) => {
    ...
    <section className="py-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {industries.slice(0, 9).map((cat, idx) => {
            const path = `/jobsnearme${cat.URL || cat.mapKey}`;
            return (
              <Link 
                key={idx} 
                to={path}
                ...
                className="group relative bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-400 transition-all duration-300 shadow-sm hover:shadow-[0_10px_40px_rgba(0,0,0,0.04)] text-left flex items-start gap-6 cursor-pointer"
              >
